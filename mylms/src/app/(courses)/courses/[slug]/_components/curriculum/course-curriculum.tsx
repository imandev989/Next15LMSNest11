      ...
      {data?.map((chapter, index) => (
        <div className="rounded-xl" key={`chapter-${chapter.id}`}>
          <h2 className="accordion-title">
            ...
            >
              <span className="h-6 w-6  drop-shadow-md rounded-full flex items-center justify-center font-bold text-sm mr-2 bg-base-content/10">
                {index}
              </span>
              <span className="mr-auto">{chapter.title}</span>
              <div className="flex gap-2 mr-3">
                <Badge variant="info">{chapter.numOfLectures} Topic </Badge>
            ...
            <div className={`accordion-content`}>
              <ol
                className={`relative pr-5 border-primary/20 ${
                  chapter.lectures.length > 1 ? "border-l-2" : ""
                }`}
              >
                {chapter.lectures.map((lecture) => (
                  <li
                    className="py-5 border-b border-base-content/10 mr-4 border-dashed last-of-type:border-0"
                    key={`lecture${lecture.title}`}
                  >
                    <span className="absolute flex items-center justify-center w-4 h-4 bg-blue-100 rounded-full -left-2  bg-primary/30"></span>
